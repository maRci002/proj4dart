...
class DatumUtils {
  static bool compareDatums(Datum source, Datum dest) {
    if (source.datum_type != dest.datum_type) {
      ...
      return false;
    } else if (source.datum_type == ConstValues.PJD_3PARAM) {
    ...
    } else if (source.datum_type == ConstValues.PJD_7PARAM) {
      return (source.datum_params[0] == dest.datum_params[0] &&
          source.datum_params[1] == dest.datum_params[1] &&
          source.datum_params[2] == dest.datum_params[2] &&
 ...
 *
 */
  static Point geodeticToGeocentric(Point p, es, a) {
    var Longitude = p.x;
    var Latitude = p.y;
   ...
   */
    if (Latitude < -ConstValues.HALF_PI &&
        Latitude > -1.001 * ConstValues.HALF_PI) {
      Latitude = -ConstValues.HALF_PI;
    } else if (Latitude > ConstValues.HALF_PI &&
        Latitude < 1.001 * ConstValues.HALF_PI) {
      Latitude = ConstValues.HALF_PI;
    } else if (Latitude < -ConstValues.HALF_PI) {
  ...
  static Point geocentricToGeodetic(Point p, double es, double a, double b) {
...
*/
  static Point geocentricToWgs84(
      Point p, int datum_type, List<double> datum_params) {
    if (datum_type == ConstValues.PJD_3PARAM) {
      // if( x[io] === HUGE_VAL )
      //    continue;
      return Point.withZ(
    ...
    } else if (datum_type == ConstValues.PJD_7PARAM) {
...
//  point to transform in geocentric coordinates (x,y,z)
  static Point geocentricFromWgs84(
      Point p, int datum_type, List<double> datum_params) {
    if (datum_type == ConstValues.PJD_3PARAM) {
      //if( x[io] === HUGE_VAL )
      //    continue;
