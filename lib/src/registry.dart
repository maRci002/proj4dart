...
class Registry {
  ...
  static final List<Ellipsoid> ellipsoids = [
    ...
    Ellipsoid.v1('NWL9D', 6378145.0, 298.25, 0.0, 'Naval Weapons Lab., 1965'),
    Ellipsoid.v1(
        'andrae', 6377104.43, 300.0, 0.0, 'Andrae 1876 (Den., Iclnd.)'),
    Ellipsoid.v1(
        'aust_SA', 6378160.0, 0.0, 298.25, 'Australian Natl & S. Amer. 1969'),
    Ellipsoid.v1('GRS67', 6378160.0, 0.0, 298.2471674270, 'GRS 67 (IUGG 1967)'),
    Ellipsoid.BESSEL,
    Ellipsoid.v1(
    ...
    Ellipsoid.v1('plessis', 6376523.0, 6355863.0, 0.0, 'Plessis 1817 France)'),
  ...
  Ellipsoid getEllipsoid(String name) {
    for (var i = 0; i < ellipsoids.length; i++) {
      if (ellipsoids[i].shortName == name) {
        return ellipsoids[i];
      }
    }
    return null;
  ...
  Map<String, dynamic> projRegistry;

  // void register( String name, Class cls, String description ) {
...
// //    register( 'nzmg', Projection.class, 'Zealand Map Grid' );
