...
class Utils {
  ...
  static double adjust_lat(double x) {
    return (x.abs() < ConstValues.HALF_PI) ? x : (x - (sign(x) * math.pi));
  }
  ...
  static int adjust_zone(int zone, double lon) {
    if (zone == null) {
  ...
  static List<double> clens_cmplx(List<double> pp, double arg_r, double arg_i) {
  ...
  static double clens(List<double> pp, double arg_r) {
  ...
  static double fL(double x, double L) {
    return 2 * math.atan(x * math.exp(L)) - ConstValues.HALF_PI;
  }

  static double gatg(List<double> pp, double B) {
  ...
  static double gN(double a, double e, double sinphi) {
    var temp = e * sinphi;
  ...
  static double imlfn(double ml, double e0, double e1, double e2, double e3) {
    double phi;
    double dphi;
    phi = ml / e0;
    for (var i = 0; i < 15; i++) {
  ...
  static Point inverseNadCvt(Point t, Point val, tb, ct) {
    if ((t.x).isNaN) {
      return val;
    }
    t.x = tb.x + t.x;
    ...
    do {
      ...
      i = i - 1;
        ...
        (dif.x).abs() > tol &&
        (dif.y).abs() >
            tol); // TODO: not sure if this while loop works correctly
    if (i < 0) {
      return val;
    }
  ...
  static double invlatiso(double eccent, double ts) {
    var phi = fL(1, ts);
    var Iphi = 0.0;
    var con = 0.0;
    do {
  ...
  static double iqsfnz(double eccent, double q) {
    var temp = 1 -
        (1 - eccent * eccent) /
            (2 * eccent) *
    ...
    if ((q.abs() - temp).abs() < 1.0E-6) {
      if (q < 0) {
        return (-1 * ConstValues.HALF_PI);
      } else {
        return ConstValues.HALF_PI;
      }
    }
  ...
  static double latiso(double eccent, double phi, double sinphi) {
    if (phi.abs() > ConstValues.HALF_PI) {
      return double.nan;
    }
    if (phi == ConstValues.HALF_PI) {
      return double.infinity;
    }
    if (phi == -1 * ConstValues.HALF_PI) {
      return double.negativeInfinity;
    }
    var con = eccent * sinphi;
    return math.log(math.tan((ConstValues.HALF_PI + phi) / 2)) +
  ...
  static Point nad_cvt(Point pin, bool inverse, ct) {
  ...
  static nadInterBreakout(indx, frct, String letter, int number, ct) {
    var inx;
    if (indx[letter] < 0) {
  ...
  static double phi2z(double eccent, double ts) {
    var eccnth = 0.5 * eccent;
    double con, dphi;
    var phi = ConstValues.HALF_PI - 2 * math.atan(ts);
    for (var i = 0; i <= 15; i++) {
      con = eccent * math.sin(phi);
      dphi = ConstValues.HALF_PI -
  ...
  static List<double> pj_enfn(double es) {
    var C00 = 1.0;
    var C02 = 0.25;
    var C04 = 0.046875;
    var C06 = 0.01953125;
  ...
  static double pj_inv_mlfn(double arg, double es, List<double> en) {
    var MAX_ITER = 20;
    var k = 1 / (1 - es);
    var phi = arg;
  ...
  static double pj_mlfn(double phi, double sphi, double cphi, List<double> en) {
    cphi *= sphi;
    sphi *= sphi;
    return (en[0] * phi -
  ...
  static double srat(double esinp, int exp) {
  ...
  static Point toPoint(List<double> array) {
  ...
  static double tsfnz(double eccent, double phi, double sinphi) {
    ...
    return (math.tan(0.5 * (ConstValues.HALF_PI - phi)) / con);
  }

  static void checkCoord(dynamic coord) {
    if (coord is double) {
      if (coord.isFinite) {
        return;
      }
  ...
  static void checkSanity(Point point) {
    checkCoord(point.x);
    checkCoord(point.y);
  }

  // TODO crs type?
  static Point adjust_axis(crs, bool denorm, Point point) {
  ...
  static void extend(dynamic destination, dynamic source) {
